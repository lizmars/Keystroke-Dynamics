...
class Digraph:
    ...
    def addModif(self,keyinput): #keyinput = [key,action,value]
        ...
        if len(self.temp) == 3:
            ...
            act2 = self.temp[1]
            act3 = self.temp[0]
            time1 = abs(float(act1[2]) - float(act2[2]))
            time2 = abs(float(act3[2]) - float(act2[2]))
            time = float(time1/time2)
            #print "time 1 = ", time1
            #print "time 2 = ", time2
            #print "time = time1/time2 = ", time
            key = act1[0] + act1[1] + act2[0] + act2[1] + act3[0] + act3[1]
            return key, time
        else:
            return None, None
    ...
    def add(self,keyinput): #keyinput = [key,action,value]
            ...
            if len(self.temp) == 2:
                act1 = self.temp.pop() # take away first value and store 2 others for next digraph
                act2 = self.temp[0]
                time = abs(float(act1[2]) - float(act2[2]))
                key = act1[0] + act1[1] + act2[0] + act2[1]
...
user = "Yuliia"
rawinput = [["40", "1", "0.02"],["40", "0", "0.025"], ["23", "1", "0.029"],
...
myprofile = Profile()
myprofile.createProfile(user, rawinput)
